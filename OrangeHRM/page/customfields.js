...
class CustomField {
    ...
    async infoclick() {
        await utilite.click(objectRepository.customField.infoLink)
        await utilite.time(5000)
    }

    async personallinkclick() {
        await utilite.click(objectRepository.customField.personalLink)
        await utilite.time(5000)
    }
    ...
    async BloodType(value) {
        ...
        const dropdown = await browser.$(objectRepository.customField.bloodTypeDropdown)
        await dropdown.waitForDisplayed()
        await dropdown.scrollIntoView({ block: 'center' })
        await dropdown.waitForClickable()
        await dropdown.click()

        const option = await browser.$(objectRepository.customField.bloodTypeOption(value))
    ...
    async test(value) {
        await utilite.enterText(objectRepository.customField.testField, value)
    }

    async savebutton() {
        await utilite.time(3000)
        await utilite.click(objectRepository.customField.saveButton)
    }
}
...
// const savebutton='//div[2]//div[2]/button'
// const utilite= new Utilities()
